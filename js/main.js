...
app.common = {
  init: function() {
    ...
    $(".header__button--search--desktop").click(function(e) {
      e.preventDefault();
      $("header").addClass("header--search-in");
      $("header").addClass("header--color");
      return setTimeout(function() {
    ...
    $("section .section__namefilters").click(function() {
      return $(this).parents("section").find(".section__filters").addClass("section__filters--show");
    });
    $("section .section__filters__headermobile .section__close").click(function() {
      return $(this).parents("section").find(".section__filters").removeClass("section__filters--show");
    });
    return $("section .section__filters__back").click(function() {
      return $(this).parents("section").find(".section__filters").removeClass("section__filters--show");
    });
  }
};
  ...
  nav: {
    ...
    enterLvl: function(el, e) {
      if (el.siblings("ul").length && $(window).width() < 1090) {
        ...
        return $(".header__secondary .header__nav__mobile").addClass("header__nav--ul-in");
      }
    },
    backLvl: function() {
      if ($(".header__secondary .header__nav--ul-in").length) {
        $(".header__secondary .header__nav__mobile").removeClass("header__nav--ul-in");
...
app.isotope8 = {
  init: function() {
    var $grid_available, $suggestionTexts, buttonFilter, buttonFilters, concatValues, debounce, filterParam, filters, generateCombinations, getUrlParameter, procesarValores, qsRegex, searchTimer, selectedFilters;
    buttonFilters = {};
    buttonFilter = void 0;
    filters = {};
    selectedFilters = {};
    searchTimer = null;
    qsRegex = void 0;
    $grid_available = $('.grid--available').isotope({
      itemSelector: '.col-xs-6',
      layoutMode: 'masonry',
      getSortData: {
        price: '.price parseInt',
        prox: '.prox parseInt',
        recient: '.recient parseInt'
      },
      sortBy: ['price', 'prox', 'recient'],
      sortBy: null,
      sortAscending: null,
      filter: function() {
    ...
    setTimeout(function() {
      return $grid_available.isotope('updateSortData').isotope();
    }, 500);
    $suggestionTexts = [];
    if ($('.section__suggestions__content').length) {
      $('.section__suggestions__content .section__suggestion').each(function() {
        var spanText;
        spanText = $(this).find('span').text();
        return $suggestionTexts.push(spanText);
      });
    }
    ...
    procesarValores = function(valores) {
      var filteredValues;
      if (Array.isArray(valores)) {
        filteredValues = valores.filter(Boolean);
    ...
    generateCombinations = function(categorias) {
      var categoria, clase, clases, i, j, len, len1, nueva_combinacion, nuevas_combinaciones, resultado, resultados;
      resultados = [[]];
      for (categoria in categorias) {
        clases = categorias[categoria];
        if (clases.length === 0) {
          continue;
        }
        nuevas_combinaciones = [];
        for (i = 0, len = resultados.length; i < len; i++) {
          resultado = resultados[i];
          for (j = 0, len1 = clases.length; j < len1; j++) {
            clase = clases[j];
            nueva_combinacion = resultado.slice();
            nueva_combinacion.push(clase);
            nuevas_combinaciones.push(nueva_combinacion);
          }
        }
        resultados = nuevas_combinaciones;
      }
      return resultados.map(function(combinacion) {
    ...
    if (filterParam) {
      setTimeout(function() {
        return $(".section__checkbox input[value='." + filterParam + "']").trigger('click');
      }, 2000);
    }
    ...
    $('.section--companies .filters').on('click', '.section__checkbox input', function() {
      var $buttonGroup, $this, combinedFilters, currentFilter, filterGroup, filterKey, filterValue, innerKey, innerValue, key, resultados, selector, value;
      $this = $(this);
      $buttonGroup = $this.parents('.button-group');
      filterGroup = $buttonGroup.attr('data-filter-group');
      currentFilter = buttonFilters[filterGroup] || {};
      if ($this.is(':checked')) {
        filterKey = $buttonGroup.attr('data-filter-group');
        filterValue = $this.parents("label").attr('data-filter');
        if (!currentFilter[filterKey]) {
          currentFilter[filterKey] = [];
        }
        currentFilter[filterKey].push(filterValue);
      } else {
        filterKey = $buttonGroup.attr('data-filter-group');
        filterValue = $this.parents("label").attr('data-filter');
        if (currentFilter[filterKey]) {
          currentFilter[filterKey] = currentFilter[filterKey].filter(function(value) {
            return value !== filterValue;
          });
        }
      }
      buttonFilters[filterGroup] = currentFilter;
      combinedFilters = {};
      for (key in buttonFilters) {
        value = buttonFilters[key];
        for (innerKey in value) {
          innerValue = value[innerKey];
          if (!combinedFilters[innerKey]) {
            combinedFilters[innerKey] = [];
          }
          combinedFilters[innerKey] = combinedFilters[innerKey].concat(innerValue);
        }
      }
      console.log(combinedFilters);
      resultados = generateCombinations(combinedFilters);
      console.log(resultados);
      selector = resultados.join(', ');
      buttonFilter = selector;
      console.log(selector);
      selectedFilters = $('.filters').find('input:checked').length;
      $('#filterCountText').text(selectedFilters + " Filtro" + (selectedFilters !== 1 ? 's' : ''));
      $grid_available.isotope();
    });
    $('.section--companies .filters-order').change(function() {
      var $this, selectedValue;
      $this = $(this);
      ...
      if (selectedValue === 'maxprox') {
        $grid_available.isotope({
          sortBy: 'prox',
          sortAscending: false
        });
      }
      if (selectedValue === 'minprox') {
        $grid_available.isotope({
          sortBy: 'prox',
          sortAscending: true
        });
      }
      if (selectedValue === 'maxrecient') {
        $grid_available.isotope({
          sortBy: 'recient',
          sortAscending: true
        });
      }
      if (selectedValue === 'minrecient') {
        $grid_available.isotope({
          sortBy: 'recient',
          sortAscending: false
        });
    ...
    $('.quicksearch').on('keyup', function() {
      var $quicksearchValue, resulttext;
      $quicksearchValue = $('.quicksearch').val();
      qsRegex = new RegExp($quicksearchValue, 'gi');
      resulttext = $quicksearchValue;
      clearTimeout(searchTimer);
      $('.section__suggestions__content').empty();
      return $grid_available.isotope({
        filter: function() {
          var $this, buttonResult, searchResult;
          $this = $(this);
          searchResult = qsRegex ? $this.text().match(qsRegex) : true;
          if (buttonFilter) {
            buttonResult = $this.is(buttonFilter);
          } else {
            buttonResult = true;
          }
          return searchResult && buttonResult;
    ...
    $('[data-clear-filters]').click(function() {
      ...
      $('.quicksearch').val("");
      $('.quicksearch').keyup();
      $(".section--companies .section__filters ").find("input").prop('checked', false);
      selectedFilters = $('.section--companies .filters').find('input:checked').length;
      $('#filterCountText').text(selectedFilters + " Filtro" + (selectedFilters !== 1 ? 's' : ''));
      return app.isotope8.init();
    });
    return $(document).on("click", ".section__suggestion", function() {
      var textsuggestion;
      textsuggestion = $(this).find("span").text();
      qsRegex = new RegExp(textsuggestion, 'gi');
      $('.quicksearch').val(textsuggestion);
      return $grid_available.isotope({
        filter: function() {
          var $this, buttonResult, searchResult;
          $this = $(this);
...
app.googleMaps = {
  init: function() {
    var lat, lng, map, mapStyle, marker;
    if ((window.google != null) && $("body").find(".section--map").length) {
      lng = parseFloat($(".section--map").attr("data-lng"));
      lat = parseFloat($(".section--map").attr("data-lat"));
      mapStyle = [
        {
...
window.initMap = app.googleMaps.init;
...
app.slider = {
  ...
  events: function(slider) {
    slider.find(".slider-nav-left").click(function() {
      return app.slider.prev(slider);
    });
    slider.find(".slider-nav-right").click(function() {
      return app.slider.next(slider);
    });
    return slider.find(".slider-bullet").click(function() {
      return app.slider.go(slider, $(this).index());
    });
  },
  ...
  go: function(slider, goto, dir) {
    var current, sliderGoto, sliderVideo;
    if (dir == null) {
      dir = false;
    }
    current = slider.find(".slide.slide-current").index();
    if (!slider.hasClass("slider-animate") && current !== goto) {
      ...
      if (sliderVideo.length) {
        sliderVideo.append(app.video.createPlayer(sliderVideo.attr("slider-video"), sliderVideo.attr("slider-video-poster")));
      }
      return app.bg.init();
    }
  },
...
app.swiper = {
  init: function() {
    ...
    return swiper2 = new Swiper('.swiper-testimonies', {
      loop: true,
      freeMode: true,
      spaceBetween: 0,
      grabCursor: true,
